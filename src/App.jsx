    ...
    <>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .panel-title{font-size:14px;font-weight:600;color:var(--muted);margin-bottom:16px;text-transform:uppercase;letter-spacing:0.5px}.panel-tasks{display:flex;flex-direction:column;gap:8px;margin-bottom:24px}.panel-task-item{display:flex;align-items:center;gap:10px;padding:12px;background:var(--bg);border-radius:var(--radius-sm);border-left:3px solid var(--border);position:relative}.panel-task-item.work{border-left-color:var(--work)}.panel-task-item.personal{border-left-color:var(--personal)}.panel-task-item.done{opacity:0.5}.panel-task-name{font-size:13px;font-weight:500;flex:1;word-wrap:break-word}.panel-task-time{font-size:11px;color:var(--muted)}.panel-task-actions{display:flex;gap:4px;margin-left:8px}.panel-action-btn{width:28px;height:28px;border-radius:6px;border:1px solid var(--border);background:var(--card);cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;transition:all 0.15s}.panel-action-btn svg{width:14px;height:14px;color:var(--muted);stroke:var(--muted)}.panel-action-btn:hover{background:var(--bg)}.panel-action-btn:first-child:hover{border-color:var(--blue);background:var(--blue-light)}.panel-action-btn:first-child:hover svg{color:var(--blue);stroke:var(--blue)}.panel-action-btn:last-child:hover{border-color:var(--danger);background:var(--danger-light)}.panel-action-btn:last-child:hover svg{color:var(--danger);stroke:var(--danger)}
          ...
          <div className="app">
            <div className="main-content">
              ...
              <div className="secondary-panel">
                    ...
                    return (
                      <div key={dateStr}>
                        ...
                        <div className="panel-tasks">
                          {dateTasks.length > 0 ? dateTasks.slice(0, 4).map(task => (
                            <div key={task.id} className={`panel-task-item ${task.category.toLowerCase()} ${task.status === 'Done' ? 'done' : ''}`}>
                              ...
                              {task.status !== 'Done' && (
                                <div className="panel-task-actions">
                                  <button className="panel-action-btn" onClick={() => handleEdit(task)} title="Edit">
                                  ...
                                  <button className="panel-action-btn" onClick={() => handleDelete(task.id)} title="Delete">
                ...
                {activeTab === 'calendar' && (<><h3 className="panel-title">{formatDate(selectedDate)} Tasks</h3><div className="panel-tasks">{selectedDateTasks.map(task => (<div key={task.id} className={`panel-task-item ${task.category.toLowerCase()} ${task.status === 'Done' ? 'done' : ''}`}><div style={{ flex: 1, minWidth: 0 }}><span className="panel-task-name">{task.task}</span><div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '4px' }}><span className="panel-task-time">{formatTime(task.timeRequired)}</span></div></div>{task.status !== 'Done' && (<div className="panel-task-actions"><button className="panel-action-btn" onClick={() => handleEdit(task)} title="Edit"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 3a2.85 2.85 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></svg></button><button className="panel-action-btn" onClick={() => handleDelete(task.id)} title="Delete"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/></svg></button></div>)}</div>))}{selectedDateTasks.length === 0 && <p style={{ color: 'var(--muted)', fontSize: '13px' }}>No tasks for this day</p>}</div></>)}
