...
const DEFAULT_SETTINGS = { dailyLimit: 480, workLimit: 360, personalLimit: 120 };
      ...
      <div ref={itemRef} className={`task-item ${task.category.toLowerCase()} ${isDone ? 'done' : ''} ${isSelectionMode && !isDone ? 'selectable' : ''}`} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} onClick={() => isSelectionMode && !isDone && onSelect(task.id)}>
        ...
        {!isSelectionMode && !isTouchDevice && showHoverActions && (
          <div className="hover-actions">
            <button className="hover-btn edit" onClick={(e) => { e.stopPropagation(); onEdit(task); }} title="Edit"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg></button>
            <button className="hover-btn delete" onClick={(e) => { e.stopPropagation(); onDelete(task.id); }} title="Delete"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6" /><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" /></svg></button>
          </div>
        )}
      </div>
    </div>
...
const TaskModal = ({ task, onSave, onClose, selectedDate }) => {
  const [formData, setFormData] = useState(task || { task: '', category: 'Work', timeRequired: 30, status: 'Pending', date: selectedDate, repeat: 'none' });
...
const AnalyticsView = ({ tasks }) => {
  ...
  return (
    <div className="analytics-view">
      <div className="analytics-card highlight"><span className="big-number">{todayDone}/{todayTotal}</span><span className="big-label">tasks completed today</span></div>
      <div className="stats-row">
        <div className="stat-card"><span className="stat-value">🔥 {streak}</span><span className="stat-label">Day Streak</span></div>
        <div className="stat-card"><span className="stat-value">{formatTime(avgDaily)}</span><span className="stat-label">Daily Avg</span></div>
        <div className={`stat-card ${weekComparison >= 0 ? 'positive' : 'negative'}`}><span className="stat-value">{weekComparison >= 0 ? '+' : ''}{weekComparison}%</span><span className="stat-label">vs Last Week</span></div>
      </div>
      <div className="chart-card"><h3>Last 7 Days</h3><div className="bar-chart">{weeklyData.map((day) => (<div key={day.date} className="bar-col"><div className="bar-stack"><div className="bar personal" style={{ height: `${(day.personalDone / maxTime) * 100}%` }} /><div className="bar work" style={{ height: `${(day.workDone / maxTime) * 100}%` }} /></div><span className="bar-day">{parseLocalDate(day.date).toLocaleDateString('en-US', { weekday: 'short' }).charAt(0)}</span></div>))}</div><div className="chart-legend"><span><span className="legend-dot work"></span> Work</span><span><span className="legend-dot personal"></span> Personal</span></div></div>
      <div className="insight-card"><span className="insight-icon">📊</span><p>Your most productive day is <strong>{dayNames[bestDayIndex]}</strong></p></div>
    </div>
  );
};
    ...
    <>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        @media(min-width:1024px){.app{max-width:none;margin:0;padding-bottom:0;flex:1}.main-content{display:flex;flex:1}.primary-panel{flex:1;max-width:600px;min-width:400px;border-right:1px solid var(--border);overflow-y:auto;height:100vh}.secondary-panel{width:320px;background:var(--card);padding:24px;overflow-y:auto;height:100vh;flex-shrink:0}.bottom-nav{display:none!important}.fab{bottom:32px;right:32px}}
        .sidebar{display:none}
        @media(min-width:1024px){.sidebar{width:var(--sidebar-width);background:var(--card);border-right:1px solid var(--border);display:flex;flex-direction:column;padding:20px 12px;flex-shrink:0;height:100vh}.sidebar-logo{display:flex;align-items:center;gap:12px;padding:8px 12px;margin-bottom:24px}.sidebar-logo span{font-size:18px;font-weight:700}.sidebar-nav{flex:1;display:flex;flex-direction:column;gap:4px}.sidebar-item{display:flex;align-items:center;gap:12px;padding:12px 16px;border:none;background:transparent;border-radius:var(--radius-sm);cursor:pointer;font-size:14px;font-weight:500;color:var(--text-secondary);text-align:left;transition:all 0.15s;font-family:inherit}.sidebar-item:hover{background:var(--bg);color:var(--text)}.sidebar-item.active{background:var(--personal-light);color:var(--personal)}.sidebar-item svg{width:20px;height:20px;flex-shrink:0}.sidebar-item .sidebar-emoji{font-size:18px;width:20px;text-align:center}.shortcut-hint{margin-left:auto;font-size:11px;color:var(--muted);background:var(--bg);padding:2px 6px;border-radius:4px;font-weight:600}.sidebar-item.active .shortcut-hint{background:rgba(16,185,129,0.2)}.sidebar-footer{border-top:1px solid var(--border);padding-top:12px;margin-top:12px}.sidebar-user{display:flex;align-items:center;gap:10px;padding:12px;margin-top:8px}.sidebar-avatar{width:32px;height:32px;border-radius:50%;object-fit:cover}.sidebar-avatar-placeholder{width:32px;height:32px;border-radius:50%;background:var(--personal);color:white;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:600}.sidebar-username{font-size:13px;font-weight:500}}
        ...
        .hover-actions{display:flex;gap:8px;margin-left:auto;padding-left:12px}.hover-btn{width:32px;height:32px;border-radius:8px;border:1px solid var(--border);background:var(--bg);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.15s}.hover-btn svg{width:16px;height:16px;color:var(--muted)}.hover-btn:hover{background:var(--card)}.hover-btn.edit:hover{border-color:var(--blue)}.hover-btn.edit:hover svg{color:var(--blue)}.hover-btn.delete:hover{border-color:var(--danger)}.hover-btn.delete:hover svg{color:var(--danger)}
          ...
          <div className="app">
            <div className="main-content">
              <div className="primary-panel">
                ...
                {activeTab === 'calendar' && !isSelectionMode && (<><div className="header"><div className="header-left"><h1>Calendar</h1></div></div><CalendarView tasks={tasks} onDateSelect={setSelectedDate} selectedDate={selectedDate} /><div className="date-nav"><button onClick={() => navigateDate(-1)}>‹</button><span>{formatDate(selectedDate)}</span><button onClick={() => navigateDate(1)}>›</button></div><MiniProgress tasks={selectedDateTasks} settings={settings} /></>)}
              ...
              <div className="secondary-panel">
                {activeTab === 'today' && (<><h3 className="panel-title">This Month</h3><MiniCalendar tasks={tasks} onDateSelect={(date) => { setSelectedDate(date); setActiveTab('calendar'); }} selectedDate={selectedDate} /><h3 className="panel-title">Upcoming</h3><div className="panel-tasks">{tasks.filter(t => t.date > today && t.status !== 'Done').slice(0, 5).map(task => (<div key={task.id} className={`panel-task-item ${task.category.toLowerCase()}`}><span className="panel-task-name">{task.task}</span><span className="panel-task-time">{formatDate(task.date)}</span></div>))}{tasks.filter(t => t.date > today && t.status !== 'Done').length === 0 && <p style={{ color: 'var(--muted)', fontSize: '13px' }}>No upcoming tasks</p>}</div></>)}
