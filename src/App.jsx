    ...
    <>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .calendar-view{padding:16px 20px;max-width:600px}.cal-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}.cal-nav-btn{width:40px;height:40px;border-radius:50%;border:1px solid var(--border);background:var(--card);cursor:pointer;display:flex;align-items:center;justify-content:center}.cal-nav-btn svg{width:20px;height:20px;color:var(--text)}.cal-title{font-size:16px;font-weight:600}.cal-weekdays{display:grid;grid-template-columns:repeat(7,1fr);margin-bottom:6px}.weekday{text-align:center;font-size:11px;font-weight:600;color:var(--muted);padding:6px 0}.cal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:3px}.cal-day{aspect-ratio:1;max-height:70px;display:flex;flex-direction:column;align-items:center;justify-content:center;border-radius:10px;cursor:pointer;position:relative;background:var(--card)}.cal-day.empty{background:transparent;cursor:default}.cal-day.today{background:var(--personal-light)}.cal-day.selected{background:var(--personal);color:white}.day-num{font-size:13px;font-weight:500}.day-indicators{display:flex;gap:2px;margin-top:3px}.indicator{width:4px;height:4px;border-radius:50%}.indicator.work{background:var(--work)}.indicator.personal{background:var(--personal)}.cal-day.selected .indicator.work,.cal-day.selected .indicator.personal{background:white}
          ...
          <div className="app">
            <div className="main-content">
              ...
              <div className="secondary-panel">
                {activeTab === 'today' && (<><h3 className="panel-title">This Month</h3><MiniCalendar tasks={tasks} onDateSelect={(date) => { setSelectedDate(date); setActiveTab('calendar'); }} selectedDate={selectedDate} />{(() => {
                  const dates = [];
                  // Last 4 days (incomplete tasks only)
                  for (let i = 4; i >= 1; i--) {
                    const d = new Date();
                    d.setDate(d.getDate() - i);
                    dates.push({ dateStr: dateToStr(d), label: formatDate(dateToStr(d)), showAll: false });
                  }
                  // Next 2 days (all tasks)
                  for (let i = 1; i <= 2; i++) {
                    const d = new Date();
                    d.setDate(d.getDate() + i);
                  ...
                  return dates.map(({ dateStr, label, showAll }) => {
                    ...
                    const displayTasks = dateTasks.slice(0, 4);
                      ...
                      <div key={dateStr}>
                        ...
                        <div className="panel-tasks">
                          {dateTasks.length > 0 ? displayTasks.map(task => (
                            <div key={task.id} className={`panel-task-item ${task.category.toLowerCase()} ${task.status === 'Done' ? 'done' : ''}`}>
                              ...
                              {task.status !== 'Done' && (
                                <div className="panel-task-actions">
                                  <button className="panel-action-btn shift" onClick={() => handleShiftSingleTask(task)} title="Shift task">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
                                  </button>
                                  <button className="panel-action-btn edit" onClick={() => handleEdit(task)} title="Edit">
                                  ...
                                  <button className="panel-action-btn delete" onClick={() => handleDelete(task.id)} title="Delete">
                          ...
                          {remainingCount > 0 && (
                            <div style={{ textAlign: 'center', padding: '8px', color: 'var(--blue)', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }} onClick={() => { setSelectedDate(dateStr); setActiveTab('calendar'); }}>
                ...
                {activeTab === 'calendar' && (<><h3 className="panel-title">{formatDate(selectedDate)} Tasks</h3><div className="panel-tasks">{selectedDateTasks.slice(0, 4).map(task => (<div key={task.id} className={`panel-task-item ${task.category.toLowerCase()} ${task.status === 'Done' ? 'done' : ''}`}><div style={{ flex: 1, minWidth: 0 }}><span className="panel-task-name">{task.task}</span><div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '4px' }}><span className="panel-task-time">{formatTime(task.timeRequired)}</span></div></div>{task.status !== 'Done' && (<div className="panel-task-actions"><button className="panel-action-btn shift" onClick={() => handleShiftSingleTask(task)} title="Shift task"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></button><button className="panel-action-btn edit" onClick={() => handleEdit(task)} title="Edit"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 3a2.85 2.85 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></svg></button><button className="panel-action-btn delete" onClick={() => handleDelete(task.id)} title="Delete"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/></svg></button></div>)}</div>))}{selectedDateTasks.length === 0 && <p style={{ color: 'var(--muted)', fontSize: '13px' }}>No tasks for this day</p>}{selectedDateTasks.length > 4 && (<div style={{ textAlign: 'center', padding: '8px', color: 'var(--blue)', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }} onClick={() => setActiveTab('calendar')}>+ Show {selectedDateTasks.length - 4} more...</div>)}</div></>)}
