    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        ...
        <div className="form-group">
          ...
          <button 
            className={`smart-schedule-btn ${isLoadingAI ? 'loading' : ''}`} 
      ...
      <div className="modal-content ai-suggestions-modal" onClick={e => e.stopPropagation()}>
        ...
        {suggestions.length === 0 ? (
          <div className="no-suggestions">
            ...
            <p style={{ fontWeight: '600' }}>No available time slots</p>
            <p style={{ fontSize: '13px', color: 'var(--muted)', marginTop: '8px', lineHeight: '1.6' }}>
              {task.date === getTodayStr() 
                ? "Your schedule is full for today. Try scheduling for tomorrow or adjust your existing tasks."
                : "No available slots on this date. Try a different date or adjust your time preferences in Settings."}
            </p>
          </div>
        ) : (
          <>
    ...
    <>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .modal-overlay{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);backdrop-filter:blur(4px);display:flex;align-items:flex-end;justify-content:center;z-index:1000;animation:fadeIn 0.2s}@keyframes fadeIn{from{opacity:0}to{opacity:1}}.modal-content{background:var(--card);border-radius:24px 24px 0 0;padding:24px;padding-bottom:calc(24px + env(safe-area-inset-bottom));width:100%;max-width:480px;max-height:90vh;overflow-y:auto;animation:slideUp 0.3s}@media(min-width:1024px){.modal-overlay{align-items:center}.modal-content{border-radius:24px;max-width:440px}}@keyframes slideUp{from{transform:translateY(100%)}to{transform:translateY(0)}}.modal-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:24px}.modal-header h2{font-size:20px;font-weight:700}.close-btn{width:36px;height:36px;min-width:36px;border-radius:50%;background:var(--bg);border:1px solid var(--border);cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;flex-shrink:0}.close-btn::before{content:"✕";font-size:16px;font-weight:500;color:var(--muted);line-height:1}.close-btn:active{background:var(--border)}.form-group{margin-bottom:20px}.form-group label{display:block;font-size:13px;font-weight:600;color:var(--text-secondary);margin-bottom:8px}.form-group input[type="text"],.form-group input[type="date"],.form-group input[type="number"],.form-group input[type="time"],.form-group select{width:100%;padding:14px 16px;border:2px solid var(--border);border-radius:var(--radius-sm);font-size:16px;font-family:inherit;background:var(--card);color:var(--text)}.form-group input::placeholder{color:var(--muted)}.form-group input:focus,.form-group select:focus{outline:none;border-color:var(--personal)}.form-row{display:flex;gap:12px}.form-row-2{display:flex;gap:12px}.form-row-2 .form-group{flex:1;margin-bottom:0}.cat-btn{flex:1;padding:14px;border:2px solid var(--border);border-radius:var(--radius-sm);background:var(--card);font-size:14px;font-weight:600;cursor:pointer;color:var(--text-secondary);font-family:inherit;transition:all 0.15s}.cat-btn:active{transform:scale(0.98)}.cat-btn.active.work{border-color:var(--work);background:var(--work-light);color:#B45309}.cat-btn.active.personal{border-color:var(--personal);background:var(--personal-light);color:#047857}.time-presets{display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap}.preset-btn{padding:10px 16px;border:2px solid var(--border);border-radius:20px;background:var(--card);font-size:13px;font-weight:600;cursor:pointer;color:var(--text-secondary);font-family:inherit;transition:all 0.15s}.preset-btn:active{transform:scale(0.95)}.preset-btn.active{background:var(--text);color:white;border-color:var(--text)}.slider-row{display:flex;align-items:center;gap:14px}.slider-row input[type="range"]{flex:1;height:6px;-webkit-appearance:none;background:var(--border);border-radius:3px}.slider-row input[type="range"]::-webkit-slider-thumb{-webkit-appearance:none;width:20px;height:20px;background:var(--text);border-radius:50%;cursor:pointer}.slider-value{font-size:14px;font-weight:600;min-width:55px;text-align:right}.timing-toggle{display:flex;gap:8px;margin-bottom:12px}.timing-btn{flex:1;padding:12px;border:2px solid var(--border);border-radius:var(--radius-sm);background:var(--card);font-size:14px;font-weight:600;cursor:pointer;color:var(--text-secondary);font-family:inherit;transition:all 0.2s}.timing-btn:active{transform:scale(0.98)}.timing-btn.active{border-color:var(--personal);background:var(--personal-light);color:#047857}.smart-schedule-btn{width:100%;padding:12px;background:linear-gradient(135deg,#667EEA 0%,#764BA2 100%);color:white;border:none;border-radius:var(--radius-sm);font-size:14px;font-weight:600;cursor:pointer;font-family:inherit;transition:all 0.2s;box-shadow:0 2px 8px rgba(102,126,234,0.3);display:flex;align-items:center;justify-content:center;gap:8px}.smart-schedule-btn:hover:not(:disabled){transform:translateY(-1px);box-shadow:0 4px 12px rgba(102,126,234,0.4)}.smart-schedule-btn:active:not(:disabled){transform:translateY(0)}.smart-schedule-btn:disabled{opacity:0.7;cursor:not-allowed}.spinner-small{width:14px;height:14px;border:2px solid rgba(255,255,255,0.3);border-top-color:white;border-radius:50%;animation:spin 0.6s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}.time-picker-section{background:var(--bg);border-radius:var(--radius-sm);padding:16px;margin-bottom:12px}.time-input{width:100%;padding:14px 16px;border:2px solid var(--border);border-radius:var(--radius-sm);font-size:16px;font-family:inherit;background:var(--card);color:var(--text)}.time-input:focus{outline:none;border-color:var(--personal)}.calculated-time{display:flex;align-items:center;gap:10px;margin-top:12px;padding:12px;background:var(--card);border-radius:var(--radius-sm);font-size:14px;color:var(--text-secondary);animation:fadeIn 0.3s}.time-icon{font-size:16px}.time-range{font-weight:600;color:var(--personal)}.conflict-warning{margin-top:12px;padding:12px;background:#FEE2E2;border:1px solid #EF4444;border-radius:var(--radius-sm);color:#DC2626;font-size:13px;font-weight:600;animation:shake 0.5s}@keyframes shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-4px)}75%{transform:translateX(4px)}}.save-btn{width:100%;padding:16px;background:var(--personal);color:white;border:none;border-radius:var(--radius-sm);font-size:16px;font-weight:600;cursor:pointer;margin-top:8px;font-family:inherit;transition:all 0.2s}.save-btn:hover{transform:translateY(-1px);box-shadow:0 2px 8px rgba(16,185,129,0.3)}.save-btn:active{transform:translateY(0)}
        .ai-suggestions-modal{max-width:500px}.ai-intro{font-size:14px;color:var(--muted);margin-bottom:20px;line-height:1.6}.no-suggestions{text-align:center;padding:40px 32px;animation:fadeIn 0.3s}.no-suggestions-icon{font-size:48px;margin-bottom:12px;animation:bounce 1s ease-in-out}@keyframes bounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-10px)}}.no-suggestions p{color:var(--text);font-size:15px;margin-bottom:4px}.suggestions-list{display:flex;flex-direction:column;gap:12px;margin-bottom:20px}.suggestion-card{padding:16px;background:var(--bg);border:2px solid var(--border);border-radius:var(--radius);cursor:pointer;transition:all 0.2s;position:relative;animation:slideUp 0.3s ease-out}.suggestion-card:nth-child(1){animation-delay:0s}.suggestion-card:nth-child(2){animation-delay:0.1s}.suggestion-card:nth-child(3){animation-delay:0.2s}.suggestion-card:hover{border-color:var(--personal);transform:translateY(-3px) scale(1.01);box-shadow:0 6px 16px rgba(0,0,0,0.12)}.suggestion-card:active{transform:translateY(-1px) scale(0.99)}.suggestion-card.best-match{border-color:var(--personal);background:var(--personal-light);box-shadow:0 2px 8px rgba(16,185,129,0.15)}.best-badge{position:absolute;top:-10px;left:12px;background:var(--personal);color:white;padding:4px 12px;border-radius:12px;font-size:11px;font-weight:700;animation:pulse 2s ease-in-out infinite}@keyframes pulse{0%,100%{box-shadow:0 0 0 0 rgba(16,185,129,0.4)}50%{box-shadow:0 0 0 4px rgba(16,185,129,0)}}.suggestion-time{display:flex;align-items:center;gap:10px;margin-bottom:12px}.suggestion-time .time-icon{font-size:20px}.suggestion-time .time-text{font-size:18px;font-weight:700;color:var(--text)}.suggestion-score{margin-bottom:12px}.score-bar-bg{height:6px;background:var(--border);border-radius:3px;overflow:hidden;margin-bottom:6px}.score-bar-fill{height:100%;background:linear-gradient(90deg,var(--personal) 0%,var(--work) 100%);border-radius:3px;transition:width 0.6s ease-out;animation:fillBar 0.6s ease-out}@keyframes fillBar{from{width:0}}.score-text{font-size:12px;color:var(--muted);font-weight:600}.suggestion-reasons{display:flex;flex-direction:column;gap:6px}.reason-item{display:flex;align-items:center;gap:8px;font-size:13px;color:var(--text-secondary);opacity:0;animation:fadeInUp 0.3s ease-out forwards}.reason-item:nth-child(1){animation-delay:0.3s}.reason-item:nth-child(2){animation-delay:0.4s}.reason-item:nth-child(3){animation-delay:0.5s}.reason-item:nth-child(4){animation-delay:0.6s}@keyframes fadeInUp{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}.reason-check{color:var(--personal);font-weight:700;font-size:14px}.capacity-warning{padding:16px;background:#FEF3C7;border:2px solid var(--work);border-radius:var(--radius);margin-bottom:16px;display:flex;align-items:start;gap:12px;animation:slideDown 0.3s ease-out}@keyframes slideDown{from{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:translateY(0)}}.warning-icon{font-size:24px;flex-shrink:0}.warning-content{flex:1}.warning-title{font-size:14px;font-weight:700;color:#92400E;margin-bottom:4px}.warning-text{font-size:13px;color:#78350F;line-height:1.6}.cancel-btn-modal{width:100%;padding:14px;background:transparent;color:var(--text-secondary);border:2px solid var(--border);border-radius:var(--radius-sm);font-size:15px;font-weight:600;cursor:pointer;font-family:inherit;transition:all 0.2s}.cancel-btn-modal:hover{background:var(--bg);border-color:var(--text-secondary)}
